...
function apiAuthenticated(req:NextRequest){const expected=process.env.API_KEY||'';const auth=req.headers.get('authorization')||'';if(!expected||!auth.startsWith('Bearer '))return false;return equal(encoder.encode(auth.slice(7)),encoder.encode(expected));}
